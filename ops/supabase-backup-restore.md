...
5. It restores only `public` plus `supabase_migrations` into the pinned raw image. The isolated `supabase_admin` session uses `SET ROLE postgres` while creating schemas so PostgreSQL applies the same owner default privileges as the hosted source, then resets to the image superuser only for trigger-safe data loading. `roles.sql` and the hosted managed-schema rows are deliberately not applied to that image.
