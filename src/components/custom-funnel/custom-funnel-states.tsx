...
export function CustomFunnelLoading() {
  const t = useT();
  return (
    <section className={SECTION} aria-busy="true" aria-label={t(CUSTOM_FUNNEL_LOADING_LABEL)}>
      <div
        aria-hidden="true"
...
export function CustomFunnelError() {
  ...
  return (
    <section className={SECTION}>
      ...
      >
        <h2 className="text-base font-bold text-text-primary">{t(CUSTOM_FUNNEL_ERROR_TEXT)}</h2>
        <button
          type="button"
...
export function CustomFunnelEmpty() {
  ...
  return (
    <div className="col-span-4 border border-border-subtle bg-surface p-4 md:col-span-8 lg:col-span-12">
      <p className="text-sm text-text-primary">{t(CUSTOM_FUNNEL_EMPTY_TEXT)}</p>
    </div>
  );
}
