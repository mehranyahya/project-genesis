...
-- ---------------------------------------------------------------------------
-- Operational catalog: grave-stone products
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Building-stone operational catalog (not yet a separate public adapter).
-- This keeps business data ready for future expansion without changing the
-- existing page contract today.
-- ---------------------------------------------------------------------------
...
create table public.building_stone_items (
  ...
  constraint building_stone_items_apps_chk check (
    applications <@ array['facade','flooring','stairs','interior_wall','countertop','other']::text[]
  ),
...
-- ---------------------------------------------------------------------------
-- Deterministic operational catalog version.
-- It intentionally covers pricing/selection data only. Portfolio/site content
-- does not alter the request-pricing version.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Request lead table. Pricing/catalog/terms validation remains server-side.
-- Tracking prefix is server configuration and is never hard-coded in schema.
-- ---------------------------------------------------------------------------
...
create table public.requests (
  ...
  constraint requests_catalog_versions_chk check (
    (client_catalog_version is null or client_catalog_version ~ '^[0-9a-f]{64}$')
    and (server_catalog_version is null or server_catalog_version ~ '^[0-9a-f]{64}$')
  ),
  ...
  constraint requests_identity_state_chk check (
    (anonymized_at is null and customer_name is not null and phone_normalized is not null)
    or
    (anonymized_at is not null and customer_name is null and phone_normalized is null
      and city is null and location_text is null and preferred_contact is null
      and preferred_contact_time is null and customer_note is null
      and request_fingerprint is null and request_fingerprint_key_id is null and ip_hash is null)
  ),
  constraint requests_grave_stone_required_chk check (
    anonymized_at is not null or request_type <> 'grave_stone'
    or (client_catalog_version is not null and server_catalog_version is not null
      and city is not null and location_text is not null and preferred_contact is not null
      and price_snapshot is not null
      and configuration_snapshot ?& array['product_id','variant_id','selected_option_ids']
      and jsonb_typeof(configuration_snapshot->'product_id') = 'string'
      and jsonb_typeof(configuration_snapshot->'variant_id') = 'string'
      and jsonb_typeof(configuration_snapshot->'selected_option_ids') = 'array'
      and char_length(btrim(configuration_snapshot->>'product_id')) between 1 and 120
      and char_length(btrim(configuration_snapshot->>'variant_id')) between 1 and 120
      and price_snapshot ?& array['client_displayed_price','server_calculated_price','price_type','includes','excludes','calculated_at']
      and jsonb_typeof(price_snapshot->'client_displayed_price') in ('number','null')
      and jsonb_typeof(price_snapshot->'server_calculated_price') in ('number','null')
      and jsonb_typeof(price_snapshot->'price_type') = 'string'
      and price_snapshot->>'price_type' in ('fixed','estimate','review')
      and case price_snapshot->>'price_type'
        when 'fixed' then
          jsonb_typeof(price_snapshot->'client_displayed_price') = 'number'
          and jsonb_typeof(price_snapshot->'server_calculated_price') = 'number'
          and (price_snapshot->>'client_displayed_price') ~ '^[0-9]+$'
          and (price_snapshot->>'server_calculated_price') ~ '^[0-9]+$'
          and (price_snapshot->>'client_displayed_price')::numeric > 0
          and (price_snapshot->>'server_calculated_price')::numeric > 0
          and price_snapshot->>'client_displayed_price' = price_snapshot->>'server_calculated_price'
        when 'estimate' then
          jsonb_typeof(price_snapshot->'client_displayed_price') = 'number'
          and jsonb_typeof(price_snapshot->'server_calculated_price') = 'number'
          and (price_snapshot->>'client_displayed_price') ~ '^[0-9]+$'
          ...
          and price_snapshot->>'client_displayed_price' = price_snapshot->>'server_calculated_price'
        when 'review' then
          jsonb_typeof(price_snapshot->'client_displayed_price') = 'null'
  ...
  constraint requests_building_stone_required_chk check (
    anonymized_at is not null or request_type <> 'building_stone'
    or (city is not null and preferred_contact is not null and needs_review = true
      ...
      and jsonb_typeof(configuration_snapshot->'shipping_required') = 'boolean'
      ...
      and (not (configuration_snapshot ? 'area_m2') or (
      ...
      and (configuration_snapshot->>'application' <> 'other'
        or (customer_note is not null and char_length(btrim(customer_note)) between 10 and 500))
      and price_snapshot is not null
      and price_snapshot ?& array['client_displayed_price','server_calculated_price','price_type','includes','excludes','calculated_at']
  ...
  constraint requests_contact_required_chk check (
    anonymized_at is not null or request_type <> 'contact'
    or (preferred_contact is not null and price_snapshot is null)
  ),
  ...
  constraint requests_referral_source_chk check (
    (not (configuration_snapshot ? 'source_type') and not (configuration_snapshot ? 'portfolio_reference_id'))
    or (request_type = 'contact'
      and configuration_snapshot ?& array['source_type','portfolio_reference_id']
      and configuration_snapshot->>'source_type' = 'portfolio'
      and configuration_snapshot->>'portfolio_reference_id' ~ '^pf-[0-9]{4,}$')
  ),
  ...
  constraint requests_text_trim_chk check (
    (customer_name is null or customer_name = btrim(customer_name))
    and (city is null or city = btrim(city))
    and (location_text is null or location_text = btrim(location_text))
    and (preferred_contact_time is null or preferred_contact_time = btrim(preferred_contact_time))
    and (customer_note is null or customer_note = btrim(customer_note))
    and terms_version = btrim(terms_version)
  ),
  constraint requests_time_order_chk check (
    updated_at >= status_changed_at and status_changed_at >= created_at
    and idempotency_expires_at = created_at + interval '24 hours'
    and (telegram_last_attempt_at is null or telegram_last_attempt_at >= created_at)
    and (telegram_next_attempt_at is null or telegram_next_attempt_at >= created_at)
    and (telegram_lease_until is null or telegram_lease_until = telegram_last_attempt_at + interval '2 minutes')
    and (telegram_failed_at is null or telegram_failed_at >= telegram_last_attempt_at)
  ),
  constraint requests_review_schedule_chk check (
    (status = 'contacted' and next_review_at is not null and review_set_at is not null
      and next_review_at >= review_set_at + interval '1 hour'
      and next_review_at <= review_set_at + interval '30 days')
    or (status <> 'contacted' and next_review_at is null and review_set_at is null)
  ),
...
begin
  ...
  if new.status is distinct from old.status then
    ...
    if new.status in ('closed','spam') and new.closed_at is null then
      new.closed_at := now();
    elsif new.status in ('new','contacted') then
      new.closed_at := null;
    end if;
  end if;
...
-- ---------------------------------------------------------------------------
-- RLS and privileges: no direct browser access. Server-side repositories and
-- Edge functions use service_role. Table Editor remains available to project
-- owners through the Supabase dashboard.
-- ---------------------------------------------------------------------------
...
alter table public.building_stone_items enable row level security;
alter table public.building_stone_items force row level security;
alter table public.building_stone_media enable row level security;
alter table public.building_stone_media force row level security;
...
revoke all on table public.products, public.product_variants, public.product_options,
  public.product_media, public.building_stone_items, public.building_stone_media,
  public.portfolio_items, public.portfolio_media, public.site_settings, public.requests
...
grant select, insert, update, delete on table public.products, public.product_variants,
  public.product_options, public.product_media, public.building_stone_items,
  public.building_stone_media, public.portfolio_items, public.portfolio_media,
